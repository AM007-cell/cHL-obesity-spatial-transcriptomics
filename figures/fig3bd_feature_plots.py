...
def panel(genes,name,width):
    n=len(genes)
    fig=plt.figure(figsize=(width,4.15))
    gs=fig.add_gridspec(4,n,height_ratios=[1,1,1,0.07],hspace=0.13,wspace=0.02,
                        left=0.10 if n<3 else 0.055,right=0.995,top=0.905,bottom=0.10)
    for j,g in enumerate(genes):
        ...
        for r,(code,lab,col) in enumerate(ROWS):
            ...
            padx=(d['px'].max()-d['px'].min())*0.04
            pady=(d['py'].max()-d['py'].min())*0.04
            ...
            if j==0:
                ax.text(-0.10,0.5,lab,transform=ax.transAxes,rotation=90,va='center',
                        ha='center',fontsize=6,fontweight='bold',color=col,linespacing=1.25)
    ...
    for e in ['pdf','png']:
        fig.savefig(f'{OUT}{name}.{e}',dpi=600,bbox_inches='tight',facecolor='white')
    plt.close(fig); print(' ->',name)

panel(['SFRP4'],'Fig4C_SFRP4_3samples',2.1)
